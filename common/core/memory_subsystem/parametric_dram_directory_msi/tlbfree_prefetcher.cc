...
IntPtr
TLBFreePrefetcher::lookup_ct_get_consumer(IntPtr current_ip, std::vector <correlation_entry> ct)
{
    bool found_recurrent=false;
    IntPtr the_first_consumer=0;
    IntPtr the_recurrent_consumer=0;
    //print ct
    ...
    for (std::vector<correlation_entry>::reverse_iterator iter=ct.rbegin(); iter!=ct.rend(); iter++)
    {
        //To get the offset for every comsumer inst, used to compute the prefetch address
        if (iter->GetProducerPC() == current_ip)
        {
            if (iter->GetProducerPC() == iter->GetConsumerPC())
            {
...
IntPtr TLBFreePrefetcher:: lookup_ct_get_offset(IntPtr producer_pc, IntPtr consumer_pc, std::vector <correlation_entry> ct)
{
    int32_t inst_offset_cn=0;
    for (std::vector<correlation_entry>::iterator iter=ct.begin(); iter!=ct.end(); iter++)
    {
        //To get the offset for every comsumer inst, used to compute the prefetch address
        if (iter->GetProducerPC() == producer_pc && iter->GetConsumerPC()==consumer_pc)
        {
...
TLBFreePrefetcher::getNextAddress(IntPtr current_address, UInt32 offset, core_id_t _core_id, DynamicInstruction *dynins, UInt64 *pointer_loads)
{
    ...
    if (dynins!=0 && dynins->num_target_reg != 0)
    {
        ...
        IntPtr producer_ip = dynins->eip;
        IntPtr consumer_ip = lookup_ct_get_consumer(producer_ip, ct);
#ifdef DEBUG
        cout<<" consumerPC for "<<hex<<dynins->eip<<" is: "<<consumer_ip<<endl;
#endif
        IntPtr inst_offset_cn=0xffffffffffffffff;
        IntPtr prefetch_address=0;
        if (consumer_ip!=0)
        {
            inst_offset_cn = lookup_ct_get_offset(producer_ip, consumer_ip, ct);
            //get consumer PC, may be multiple
            if(inst_offset_cn != 0xffffffffffffffff)
            {
                prefetch_address = dynins->target_reg[dynins->num_target_reg-1] + inst_offset_cn;
            }
            bool address_found = false;
            // But stay within the page if requested
            if ((!stop_at_page || ((prefetch_address & PAGE_MASK) == (current_address & PAGE_MASK))) && addresses.size() <= 1)
            {
                if (prefetch_address > 0xfffff && !only_count_lds)
                {
                    ...
                    if (prefetch_address!=current_address)
                    {
                        for(std::vector<IntPtr>::iterator it = addresses.begin(); it != addresses.end(); ++it)
                        {
                            ...
                            //  even not the same address, maybe it's the same cache line, check
        ...
        while(addresses.size()<=num_prefetches && (consumer_ip!=0))
        {
            producer_ip = consumer_ip;
            consumer_ip = lookup_ct_get_consumer(producer_ip, ct);
#ifdef DEBUG
            cout<<" consumerPC for "<<hex<<producer_ip<<" is : "<<consumer_ip<<endl;
#endif
            inst_offset_cn=0xffffffffffffffff;
            if (consumer_ip!=0)
            {
                inst_offset_cn = lookup_ct_get_offset(producer_ip, consumer_ip, ct);
            }
#ifdef DEBUG
            cout<<" offset is "<<inst_offset_cn<<endl;
#endif
            if(inst_offset_cn != 0xffffffffffffffff)
