...
LinkednewPrefetcher::LinkednewPrefetcher(String configName, core_id_t _core_id, UInt32 _shared_cores, void * cache_cntlr)
   ...
   , flows_per_core(Sim()->getCfg()->getBoolArray("perf_model/" + configName + "/prefetcher/linkednew/flows_per_core", core_id))
   , num_prefetches(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/num_prefetches", core_id))
   , num_flattern_prefetches(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/num_flattern_prefetches", core_id))
   , stop_at_page(Sim()->getCfg()->getBoolArray("perf_model/" + configName + "/prefetcher/linkednew/stop_at_page_boundary", core_id))
   ...
   , potential_producer_window_size(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/potential_producer_window_size", core_id))  /*those param are only used to limit the size of the queue.*/
   , correlation_table_size(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/correlation_table_size", core_id))
   , correlation_table_dep_size(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/correlation_table_dep_size", core_id))
   , prefetch_request_queue_size(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/prefetch_request_queue_size", core_id))
   , prefetch_buffer_size(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/linkednew/prefetch_buffer_size", core_id))
{
...
#if 0
    /* For linkednew prefetcher, create PPW/CT/PRQ/PB */
...
int32_t LinkednewPrefetcher::DisassGetOffset(std::string inst_disass)
{
...
LinkednewPrefetcher::getNextAddress(IntPtr current_address, UInt32 offset, core_id_t _core_id, DynamicInstruction *dynins, UInt64 *pointer_loads, UInt64* multi_consumer_loads, UInt64* pointer_stores, IntPtr target_reg)
{
    ...
    //if (dynins!=0 && target_reg != 0 )
    {
        ...
        {
            MYLOG("After push back in prefetch address: 0x%lx, dynins->memory_info[0].dir is %d", *it, dynins->memory_info[0].dir);
