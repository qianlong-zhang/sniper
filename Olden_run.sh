...
do
    ...
    echo $bench
    for config in $CONFIG_0 $CONFIG_1 $CONFIG_2 $CONFIG_3 $CONFIG_4 $CONFIG_5 $CONFIG_6 $CONFIG_7 $CONFIG_8 $CONFIG_9 $CONFIG_10 $CONFIG_11 $CONFIG_12 $CONFIG_13  $CONFIG_14 $CONFIG_15 $CONFIG_16 $CONFIG_17 $CONFIG_18 $CONFIG_19
    do
        echo
        echo
        OUT_PATH=$CURRENT_PATH/output_test/${bench}${DATE_NAME}_${PAGE_SIZE}/$config
        #echo $OUT_PATH
        if [ -d $OUT_PATH ];
        then
